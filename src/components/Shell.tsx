...
/// Static fallback Razorpay URL — used only when the create-payment-link
/// edge function isn't available (function not deployed yet, network
/// failure, dev install with no Supabase). The preferred path is the
/// dynamic per-user link generated by `requestUpgradeUrl` below, which
/// embeds the user's uuid in Razorpay's `notes` so the webhook can
/// auto-flip plan_tier without manual SQL.
const RAZORPAY_FALLBACK =
...
/// Ask the create-payment-link edge function for a fresh per-user
/// Razorpay payment URL. Returns the static fallback if the function
/// errors out — better to send the user to a generic payment page
/// than to silently fail the Upgrade click.
async function requestUpgradeUrl(): Promise<string> {
  try {
    ...
    if (error) {
      console.error("[shell] create-payment-link errored:", error);
      return RAZORPAY_FALLBACK;
    }
    const url = data?.url?.trim();
    if (!url) {
      console.error("[shell] create-payment-link returned no url");
      return RAZORPAY_FALLBACK;
    }
    return url;
  } catch (e) {
    console.error("[shell] create-payment-link threw:", e);
    return RAZORPAY_FALLBACK;
  }
}
            ...
            <div className={`pf-usage-card ${usage.limitReached ? "limit-reached" : ""}`}>
              ...
              {usage.limitReached && (
                <button
                  ...
                  onClick={async () => {
                    // Ask the edge function for a per-user payment
                    // link first; falls back to the static URL if
                    // anything goes wrong. Either way openUrl is what
                    // actually launches the browser.
                    const url = await requestUpgradeUrl();
                    openUrl(url).catch((e) =>
